...
#'
#' This function groups overlapping intervals based on its start and end.
#' It assigns a unique group ID to each set of overlapping intervals.
...
#' @param .group_col The name of the column that will indicate the group ID for overlapping intervals.  Default is `int_grp_id`.
#'
#' @return A `data.table` with the original data and an additional column indicating
#'         the group ID for overlapping intervals (the `.group_id` column).
#'
#' @examples
#' \dontrun{
#' data <- data.frame(
